"""
30-seed rerun of the main Problem C comparison (random vs greedy vs UCB) with
bootstrap CIs and paired Wilcoxon tests. This provides the statistical rigour
...
def run(strategy, seed):
    ...
    for _ in range(BUDGET):
        ...
        if strategy == 'random':
            ...
            score = mean.copy(); score[~mask] = -np.inf
            choice = int(np.argmax(score))
        elif strategy == 'ucb':
            score = mean + KAPPA * np.sqrt(var); score[~mask] = -np.inf
            choice = int(np.argmax(score))
        pt = all_pts[choice]
...
if __name__ == "__main__":
    ...
    for strat in ['random', 'greedy', 'ucb']:
        rmses = [run(strat, s) for s in range(N_SEEDS)]
        raw[strat] = rmses
        m, lo, hi = bootstrap_ci(rmses)
        results[strat] = {'rmse_median': m, 'rmse_ci_lo': lo, 'rmse_ci_hi': hi, 'raw': rmses}
        print(f"{strat}: RMSE median={m:.4f} CI=[{lo:.4f}, {hi:.4f}]")

    print("\nPaired Wilcoxon signed-rank:")
    for a, b in [('ucb', 'random'), ('ucb', 'greedy'), ('random', 'greedy')]:
        stat, p = stats.wilcoxon(raw[a], raw[b])
        print(f"  {a} vs {b}: W={stat:.2f}  p={p:.6f}")
        results[f"wilcoxon_{a}_vs_{b}"] = {'W': float(stat), 'p_value': float(p)}

    with open('problem_c_30seeds.json', 'w') as f:
